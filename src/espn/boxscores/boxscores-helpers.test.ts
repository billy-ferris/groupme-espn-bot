import { roundNumber } from "./boxscores-helpers";

describe("boxscores helpers", () => {
  describe("roundNumber", () => {
    test.each([
      ...
      ({ numberToRound, placesToRound, roundedNumber }) => {
        expect(roundNumber(numberToRound, placesToRound)).toEqual(
          roundedNumber
        );
      }
    );
    ...
    it("should round to whole number if places argument is absent", () => {
      expect(roundNumber(1.634)).toEqual(2);
    });
  });
});
